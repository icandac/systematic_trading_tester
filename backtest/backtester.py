...
def run_backtest(df: pd.DataFrame):
    ...
    returns:
        pd.DataFrame: DataFrame with additional columns for unrealized PnL and returns.
    """
    pm = PortfolioManager()
    rm = RiskManager()
    ...
    for index, row in df.iterrows():
        ...
        if current_signal != 0 and rm.check_risk(current_price, current_signal):
            ...
            pm.update_position(signal=current_signal, price=current_price, quantity=quantity)
    ...
    # For simplicity, let's get returns from price only (no leveraged returns here)
